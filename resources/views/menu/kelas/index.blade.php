            ...
            <div class="lg:col-span-2 bg-white rounded-xl shadow-sm p-6">
                <div class="flex flex-col justify-center mb-4">
                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                        <h2 class="text-lg font-semibold text-gray-800">Tabel Kelas</h2>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse ($data as $user)
                                    <tr class="user-row hover:bg-gray-50 transition-colors">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 nama_kelas">{{ $user->nama_kelas }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 nama_jurusan">{{ $user->jurusan->nama_jurusan }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 angkatan">{{ $user->angkatan }}</td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex gap-3 items-center justify-center w-full">
                                                <a href="{{ route('Kelas.edit', $user->id) }}" 
                                    ...
                                    <x-modal name="confirm-delete-{{ $user->id }}" :show="false" maxWidth="sm">
                                        <div class="p-6">
                                            ...
                                            <p class="mt-2 text-sm text-gray-600">
                                                Apakah Anda yakin ingin menghapus Kelas <b>{{ $user->nama_kelas }}</b>?
                                            </p>
                                            <div class="mt-6 flex justify-end space-x-3">
                                                ...
                                                <form method="POST" action="{{ route('Kelas.destroy', $user->id) }}">
                                                    @csrf
                                                    @method('DELETE')
                                ...
                                @empty
                                    <tr>
                                        <td colspan="6" class="px-6 py-4 text-center text-gray-500">Tidak ada data Kelas</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
    ...
    <x-modal name="add-modal" focusable>
        <form method="POST" action="{{ route('Kelas.store') }}" class="p-6">
            ...
            <h2 class="text-lg font-medium text-gray-900 mb-4">
                <span class="text-indigo-600">Tambah Kelas Baru</span>
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <select id="jurusan" name="jurusan" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                ...
                <div>
                    ...
                    <x-text-input id="angkatan" placeholder="2022" name="angkatan" type="text" class="mt-1 block w-full" required oninput="this.value = this.value.replace(/[^0-9]/g, '')" />
    ...
    <script>
        ...
        function searchTable() {
            ...
            filteredRows = allRows.filter(row => {
                const namaKelas = row.querySelector('.nama_kelas').textContent.toLowerCase();
                const namaJurusan = row.querySelector('.nama_jurusan').textContent.toLowerCase();
                const angkatan = row.querySelector('.angkatan').textContent.toLowerCase();
                const createdAt = row.querySelector('.created_at').textContent.toLowerCase();
                
                return namaKelas.includes(searchTerm) || 
                       namaJurusan.includes(searchTerm) || 
